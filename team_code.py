...
def grid_lead_removal(image, lead_threshold, grid_threshold):
    
    gray_image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
    kernel = np.ones((3,3),np.uint8)
    _, only_lead = cv2.threshold(gray_image, lead_threshold, 255, cv2.THRESH_BINARY_INV)
    dilated = cv2.dilate(only_lead, kernel, iterations = 1)
...
def extract_features(record):
    ...
    clean_signal_image = grid_lead_removal(rotated_image, 20, 50)
